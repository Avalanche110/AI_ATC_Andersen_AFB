...
function AI_ATC:TaxiSubMenu(Alias)
  ...
  elseif Helo==true then
    MENU_GROUP_COMMAND:New( Group, "Request Taxi clearance", AI_ATC_Ground_Menu, function()AI_ATC:TaxiClearance(Alias) end, Group)
    MENU_GROUP_COMMAND:New( Group, "Request Hover check", AI_ATC_Ground_Menu, function()AI_ATC:HoverCheck(Alias) end, Group)
...
function AI_ATC:NavigationSubMenu(Alias, Modifier)
  ...
  if not ClientData or not ClientData.Unit then
    env.warning(("AI_ATC:NavigationSubMenu – bad ClientData for '%s'"):format(Alias))
    return
  end
    ...
    if branch == "Helo" then
      for _, navpoint in ipairs(navTable) do
        if navpoint and navpoint.GetName then
          ...
          MENU_GROUP_COMMAND:New(Group, "Vectors for "..name, VectorMenu, function() AI_ATC:NavAssist(Alias, name, Transmitter) end, Group)
        end
      end

    else
      for i = 1, math.min(#navTable, 2) do
  ...
  elseif Procedure == "IFR" then
    -- IFR-specific menu items go here when ready.
  end
...
function AI_ATC:LandingSubMenu(Alias, Modifier)
  ...
  if Helo~=true then
    ...
    MENU_GROUP_COMMAND:New(Group, "Straight In", TowerMenu, function() AI_ATC:HeloRequestLanding(Alias) end, Group)
  end
...
function AI_ATC:IFRApproachType1(Alias, Type)
  ...
  if AI_ATC:FunctionDelay(Alias, function() AI_ATC:IFRApproachType1(Alias) end, Transmitter)==false then
    return
  end
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function AI_ATC:IFRApproachType2(Alias, Type)
  ...
  local Flight = ClientData.Flight
  local CallsignSub = Flight and FlightCallsign or Callsign
  local Delay = math.random(1.5, 2.0)
  ...
  if AI_ATC:FunctionDelay(Alias, function() AI_ATC:IFRApproachType2(Alias, Type) end, Transmitter)==false then
    return
  end
...
function AI_ATC:IFRApproachType3(Alias, Type, Call, Base)
  ...
  if Call ~= true then
    USERSOUND:New("RADIO_TRANS_START.ogg"):ToUnit(Unit)
    if AI_ATC:FunctionDelay(Alias, function() AI_ATC:IFRApproachType3(Alias, Type) end, Transmitter) == false then
      return
    end
  end
  ...
  local SchedulerObject, SchedulerObject2, TerminalRange
  local AudioFile2     = approachTable[Type]
  local Key            = 2 
  local UnitCoord, Distance
  ...
  local function UpdateVariables()
    ...
    if data.Offset then
      ...
      if leg then
        Final   = leg.Navpoint
        Heading = tonumber(leg.Heading)
      end
    end
    if not Heading then Heading = AI_ATC:CorrectHeading(RunwayHeading + 180) end
...
function AI_ATC:ApproachReportFix(Alias, Report)
  ...
  local UnitCoord, Turn, Heading, Altitude, AltitudeSub, ApproachFile, ApproachSub, SchedulerObject, RunwayHeading, localizerSub, ApproachTypeFile

  SCHEDULER:New(nil, function()
    UnitCoord = Unit:GetCoordinate()
    ApproachFile = ApproachTable[Type]
    ApproachTypeFile = LocaliserType[Type]
    if Type=="HI-TAC X" then
      Altitude = "2.6"
      AltitudeSub = "2600"
      ApproachSub = "17 DME Arc to final, Cleared for the approach."
    elseif Type=="HI-ILS Y" then
      Altitude = "2.6"
      AltitudeSub = "2600"
      ApproachSub = string.format("17 DME Arc to final. Cleared ILS approach for runway %s.", Runway)
    elseif Type=="ILS" then
      Altitude = "2.6"
      AltitudeSub = "2600"
      ApproachSub = string.format("until established on the localizer. Cleared ILS approach for runway %s, 17 DME Arc to final.", Runway)
    elseif Type=="TACAN" then
      Altitude = "2.6"
      AltitudeSub = "2600"
      ApproachSub = "until established on the TACAN final, Cleared for the approach."
    end
  end,{}, 0.5)
...
function AI_ATC:ApproachManager(Alias, Type, Missed)
  ...
  local UnitCoord,AirbaseCoord, ApproachBearing, CardinalBearing
  ...
  SCHEDULER:New(nil, function()
    ...
    if data then
      ...
      local override = data.Offset.Navpoint
      if override then
        Direction, Offset = CalculateTurnDirection(UnitCoord)
  ...
  local function Execute()
    ...
    if Type == "HI-TAC X" or Type == "HI-ILS Y" then
      AlternateMessage()
      SCHEDULER:New(nil, function()
        AI_ATC:Push_Tower(Alias)
      end, {}, 12)
...
function AI_ATC:ApproachLocalizer(Alias, Type)
  ...
  SchedulerObject = SCHEDULER:New(nil, function()
    if Unit and Unit:IsAlive() then
      ...
      if (Angle >= 5 and Angle <= 15) and Range <= 55560 then
        if AI_ATC:FunctionDelay(Alias, nil, Transmitter)==true then
          Execute()
        end
        SchedulerObject:Stop()
      elseif Angle < 5 and Range <= 27780 then
        if AI_ATC:FunctionDelay(Alias, nil, Transmitter)==true then
          Execute(true)
        end
        SchedulerObject:Stop()
...
function AI_ATC:RadarManager(Alias, Type, Auto)
  ...
  SCHEDULER:New(nil, function()
    ...
    local Coord1 =  Zone_1:GetCoordinate()
    Coord1:MarkToAll("Zone_1")
    local Coord2 = Zone_2:GetCoordinate()
    Coord2:MarkToAll("Zone_2")
    local Coord3 = Zone_3:GetCoordinate()
    Coord3:MarkToAll("Zone_3")
...
function AI_ATC:Push_Tower(Alias)
  ...
  SchedulerObject = SCHEDULER:New(nil, function()
    ...
    local CorrectedHeading = AI_ATC:AngularDifference(Heading, RunwayHeading)
    local FunctionDelay = AI_ATC:FunctionDelay(Alias, nil, Transmitter)
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function AI_ATC:HeloRequestLanding(Alias)
  local Transmitter = "Tower"
  local RadioObject = TOWER_RADIO
  local Title = string.format("%s %s", AI_ATC.Airbase, Transmitter)
  local ClientData = ATM.ClientData[Alias]
  ...
  local AirbaseCoord, AirbaseVec2, AirbaseZone, Height, WindDirection, WindSpeed, LandingZone, Runway, SchedulerObject
  
  if AI_ATC:FunctionDelay(Alias, function() AI_ATC:HeloRequestLanding(Alias) end, Transmitter)==false then
    return
  end
  ...
  local function Message()
    SCHEDULER:New(nil, function()
      AI_ATC:RepeatLastTransmission(Alias, function()Message() end)
      AI_ATC:ChannelOpen(12, Transmitter, Alias)
      local Subtitle = string.format("%s: %s, Make straight in approach to runway %s. Wind %s at %s. Cleared to land. ", Title, CallsignSub, Runway, WindDirection, WindSpeed)
      RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3, Subtitle, 7)
      AI_ATC:Callsign(Callsign, RadioObject, Transmitter, Flight)
      RadioObject:NewTransmission("HeloLanding.ogg", 1.858, "Airbase_ATC/Ground/SoundFiles/", nil, 0.1)
      AI_ATC:Runway(Runway, RadioObject, Transmitter)
      RadioObject:NewTransmission("Wind.ogg", 0.583, "Airbase_ATC/Ground/SoundFiles/", nil, 0.2)
      AI_ATC:ReadHeading(WindDirection, RadioObject, Transmitter)
...
function AI_ATC:Report5MileInital(Alias, audio)
  ...
  local function Message()
    SCHEDULER:New(nil, function()
      AI_ATC:RepeatLastTransmission(Alias, function()Message() end)
      AI_ATC:ChannelOpen(12, Transmitter, Alias)
      local Subtitle = string.format("%s: %s, %s. Your number %s for runway %s, %s break mid field. Report base.", Title, CallsignSub, Title, LandingNo, Runway, Break)
      RadioObject:NewTransmission("DeadAir.ogg", 0.100, "Airbase_ATC/Ground/SoundFiles/", nil, 0.3, Subtitle, 7)
      AI_ATC:Callsign(Callsign, RadioObject, Transmitter, Flight)
      AI_ATC:AirbaseName(AI_ATC.Airbase, RadioObject, Transmitter)
      RadioObject:NewTransmission("Tower.ogg", 0.384, "Airbase_ATC/Ground/SoundFiles/", nil, nil)
      ...
      AI_ATC:Runway(Runway, RadioObject, Transmitter)
      if Break == "Left" then
        RadioObject:NewTransmission("LeftBreakMid.ogg", 1.289, "Airbase_ATC/Ground/SoundFiles/", nil, 0.01)
      elseif Break == "Right" then
        RadioObject:NewTransmission("RightBreakMid.ogg", 1.277, "Airbase_ATC/Ground/SoundFiles/", nil, 0.01)
      end
...
function AI_ATC:GoAround(Alias, audio)
  ...
  local InstructionTable = {
    ...
    ["VFR"] = { 
      ["06L"] = { filename = "UnableNorth.ogg", duration = 3.460, subtitle = "Unable closed traffic. Break out and report North Point." },
      ["06R"] = { filename = "UnableSouth.ogg", duration = 3.11, subtitle = "Unable closed traffic. Break out and report South Point."  },
      ["24L"] = { filename = "UnableSouth.ogg", duration = 3.11, subtitle = "Unable closed traffic. Break out and report South Point." },
      ["24R"] = { filename = "UnableNorth.ogg", duration = 3.460, subtitle = "Unable closed traffic. Break out and report North Point."  },
    },
  }
...
function AI_ATC:HeloLandingManager(Alias)
  ...
  local Procedure, Exit, AirbaseCoord, AirbaseVec2, AirbaseZone, SchedulerObject, Runway
