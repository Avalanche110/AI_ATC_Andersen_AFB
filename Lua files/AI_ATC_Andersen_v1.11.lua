...
function AI_ATC:RangeControlFunctionMenu(Alias)
  ...
  MENU_GROUP_COMMAND:New(Group, "Point Alpha", GateMenu, function() AI_ATC:Vector(Alias, "Alpha") end, Group)
  MENU_GROUP_COMMAND:New(Group, "Point Bravo", GateMenu, function() AI_ATC:Vector(Alias, "Bravo") end, Group)
  MENU_GROUP_COMMAND:New(Group, "Point Charlie", GateMenu, function() AI_ATC:Vector(Alias, "Charlie") end, Group)
  MENU_GROUP_COMMAND:New(Group, "Point Delta", GateMenu, function() AI_ATC:Vector(Alias, "Delta") end, Group)
  MENU_GROUP_COMMAND:New(Group, "Point Echo", GateMenu, function() AI_ATC:Vector(Alias, "Echo") end, Group)
  MENU_GROUP_COMMAND:New(Group, "Point Foxtrot", GateMenu, function() AI_ATC:Vector(Alias, "Foxtrot") end, Group)
  MENU_GROUP_COMMAND:New(Group, "Point Golf", GateMenu, function() AI_ATC:Vector(Alias, "Golf") end, Group)
  MENU_GROUP_COMMAND:New(Group, "Point Hotel", GateMenu, function() AI_ATC:Vector(Alias, "Hotel") end, Group)
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function AI_ATC:VectorToFinal(Alias, Tbl)
  local Transmitter = "Approach"
  local CleintData = ATM.ClientData[Alias]
  local Group = CleintData.Unit:GetGroup()
  local AI_ATC_Menu = CleintData.ParentMenu
...
function AI_ATC:LandingSubMenu(Alias, Modifier)
  ...
  if Helo~=true then
    ...
    MENU_GROUP_COMMAND:New(Group, "Straight In", TowerMenu, function() AI_ATC:HeloRequestLanding(Alias, "Straight In") end, Group)
    MENU_GROUP_COMMAND:New(Group, "Closed traffic", TowerMenu, function() AI_ATC:HeloRequestLanding(Alias, "Closed traffic") end, Group)
  end
...
function AI_ATC:ClearanceDelivery(Alias, Audio)
  ...
  USERSOUND:New("RADIO_TRANS_START.ogg"):ToUnit(Unit)
  local Squawk, FirstDigit, Data, SoundFile, Duration, Instruction
  
  SCHEDULER:New(nil, function()
    AI_ATC:RepeatLastTransmission(Alias, function()AI_ATC:ClearanceDelivery(Alias, true) end)
    Data = DepartureTable[Runway]
    SoundFile = Data.file
  ...
  SCHEDULER:New(nil, function()
    ...
    Subtitle = string.format("%s: Squawk %s sequential, departure frequency 239.3. Contact Andersen Ground Prior to taxi.", Title, Squawk)
    ...
    RadioObject:NewTransmission("Sequential_Andersen.ogg", 5.933, "Airbase_ATC/Clearance/SoundFiles/", nil, 0.05)
  end,{}, Delay )
...
function AI_ATC:RangeCheckIn(Alias, String, Audio)
  ...
  SCHEDULER:New(nil, function()
    AI_ATC:ChannelOpen(8, Transmitter, Alias)
    local Subtitle1 = string.format("%s: %s, %s. %s. Range altimeter %s.", Title, Callsign, Title, Instruction, QNH)
...
function AI_ATC:IFRApproachType3(Alias, Type, Call, Base)
  ...
  local function UpdateVariables()
    ...
    if data.Offset then
      ...
      AI_ATC:VectorToFinal(Alias, offsetTbl)
...
function AI_ATC:ApproachReportFix(Alias, Report, Missed)
  ...
  local SchedulerObject, SchedulerObject1, SchedulerObject2, ApproachData
...
function AI_ATC:ApproachBackToRadar(Alias, Type)
  ...
  local RunwayData = {
    ["06L"] = {Heading = "336", Altitude = "3", PatternAlt = 3000},
    ["06R"] = {Heading = "156", Altitude = "3", PatternAlt = 3000},
    ["24L"] = {Heading = "156", Altitude = "3", PatternAlt = 3000},
    ["24R"] = {Heading = "336", Altitude = "3", PatternAlt = 3000}
  }
...
function AI_ATC:Report5MileInital(Alias, audio)
  ...
  local function Message()
    SCHEDULER:New(nil, function()
      ...
      AI_ATC:Runway(Runway, RadioObject, Transmitter)
      if Break == "Left" then
        RadioObject:NewTransmission("LeftBreakMid.ogg", 1.300, "Airbase_ATC/Ground/SoundFiles/", nil, 0.01)
      elseif Break == "Right" then
        RadioObject:NewTransmission("RightBreakMid.ogg", 1.231, "Airbase_ATC/Ground/SoundFiles/", nil, 0.01)
      end
      RadioObject:NewTransmission("ReportBase.ogg", 0.848, "Airbase_ATC/Ground/SoundFiles/", nil, 0.2)
    end, {}, Delay )
  end
...
function AI_ATC:Start()
  local Subtitle = "********************************************AI_ATC(Andersen AFB) v2.9.17.11733 HAS STARTED********************************************************************"
  env.info(Subtitle)
